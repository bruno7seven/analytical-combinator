script.on_event(defines.events.on_gui_opened, function(event)
    if event.gui_type == defines.gui_type.entity then
        ...
        if entity and entity.name == "analytical-combinator" then
...
function create_custom_gui(player, entity)
    ...
    titlebar.add({
        type = "sprite-button",
        name = "analytical-combinator-close-button",
    ...
    local cpu = storage.analytical_combinators[entity.unit_number].cpu
    ...
    textbox.style.font = "ac-mono-14"

    content.add({
        type = "button",
        name = "analytical-combinator-save-button",
        caption = "Save",
    })
...
script.on_event(defines.events.on_gui_click, function(event)
    if event.element.name == "analytical-combinator-close-button" then
        event.element.parent.parent.destroy()
    elseif event.element.name == "analytical-combinator-save-button" then
        ...
        storage.analytical_combinators[unit_number].cpu:update_code(updated_code)

        -- reset errors label in GUI
        for _, player in pairs(game.players) do
            local gui_name = "analytical_combinator_gui_" .. unit_number
...
script.on_event(defines.events.on_gui_closed, function(event)
    if event.element and event.element.name == "analytical_combinator_gui" then
